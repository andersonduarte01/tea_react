...
import { criarProfissional, atualizarFotoProfissional } from '../services/profissionalService';
import { ApiError } from '../services/httpClient';
import { CriarProfissionalInput } from '../types/profissional';
...
// ─── Tela ─────────────────────────────────────────────────────────────────────
export function NovoProfissionalScreen() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const input: CriarProfissionalInput = {
        nome:     form.nome.trim(),
      ...
      const prof = await criarProfissional(input);

      if (photoUri) {
        try {
          await atualizarFotoProfissional(prof.id, photoUri);
        } catch {
          // foto falhou mas profissional foi criado — segue normalmente
        }
      }
  ...
  return (
    <SafeAreaView style={s.root} edges={['top']}>
      ...
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>Novo Profissional</Text>
      </View>
        ...
        >
          <View style={s.card}>
            ...
            <InputField
              ...
              placeholder="Dr. Carlos Lima"
              error={errors.nome} capitalize="words"
            />

            <InputField
              label="Função / Especialidade" required
              value={form.funcao} onChange={v => set('funcao', v)}
              placeholder="Ex: Psiquiatra, Fonoaudiólogo"
        ...
        >
          {loading ? (
            ...
            <>
              ...
              <Text style={s.submitText}>Cadastrar profissional</Text>
            </>
          )}
        </TouchableOpacity>
      </View>
