...
type ApiResponse = FuncionarioAPI[] | { results: FuncionarioAPI[] };
...
export function FuncionarioLista() {
  ...
  const carregar = useCallback(async (isRefresh = false) => {
    ...
    setErro(null);
    try {
      const json = await api.get<ApiResponse>('/api/v1/funcionario/');
      setDados(Array.isArray(json) ? json : (json.results ?? []));
    } catch (err: unknown) {
      if (err instanceof SessionExpiredError) {
        setErro('Sessão expirada. Faça login novamente.');
      } else {
        setErro((err as any)?.message ?? 'Erro ao carregar funcionários.');
      }
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  if (loading) {
    return (
      <View style={s.stateWrap}>
        ...
        <Text style={s.stateText}>Carregando funcionários...</Text>
      </View>
    );
  }
      ...
      {filtrados.length === 0 ? (
        <View style={s.stateWrap}>
          ...
          <Text style={s.emptySubtitle}>
            {busca ? `Sem resultados para "${busca}"` : 'Adicione funcionários para começar'}
          </Text>
        </View>
      ) : (
        <ScrollView
