...
import {
  ...
  validateForm, buildBody, apiToForm,
  TimeField, NumberField, DaysSelector, ToggleRow,
} from './agendaForm';

type Nav   = NativeStackNavigationProp<AppStackParams>;
type Route  = RouteProp<AppStackParams, 'EditarAgenda'>;

export function EditarAgendaScreen() {
  ...
  const carregar = useCallback(async () => {
    ...
    setErro(null);
    try {
      const data = await api.get(`/api/v1/agenda/profissionais/${params.agendaId}/`);
      setForm(apiToForm(data));
    } catch (e: unknown) {
      setErro((e as any)?.message ?? 'Erro ao carregar agenda.');
    } finally {
      setLoading(false);
    }
  }, [params.agendaId]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.patch(`/api/v1/agenda/profissionais/${params.agendaId}/`, body);
      ...
      if (e instanceof SessionExpiredError) {
        ...
        Alert.alert('Erro', (e as any)?.message ?? 'Não foi possível atualizar a agenda.');
      }
    } finally {
      setSaving(false);
    }
  ...
  const Header = () => (
    <View style={s.header}>
      ...
      <View style={{ flex: 1 }}>
        <Text style={s.headerTitle}>Editar Agenda</Text>
...
const s = StyleSheet.create({
  ...
  stateText: { fontSize: 13, color: C.textMuted, marginTop: 12 },
  errText:   { fontSize: 14, color: C.error, textAlign: 'center', marginBottom: 16 },
