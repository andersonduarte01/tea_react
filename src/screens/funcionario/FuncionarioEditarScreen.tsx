...
type Route = RouteProp<AppStackParams, 'FuncionarioEditar'>;
...
// ─── Screen ───────────────────────────────────────────────────────────────────
export function FuncionarioEditarScreen() {
  ...
  const set = (key: keyof FuncForm) => (v: string) =>
    setForm(f => ({ ...f, [key]: v }));
  ...
  const carregar = useCallback(async () => {
    ...
    setErro(null);
    try {
      const data = await api.get<any>(`/api/v1/funcionario/${params.id}/`);
  ...
  const uploadFoto = async (uri: string) => {
    ...
    try {
      ...
      await api.patchMultipart(`/api/v1/funcionario/${params.id}/foto/`, formData);
      setLocalPhotoUri(uri);
      Alert.alert('Foto atualizada', 'A foto do funcionário foi atualizada com sucesso.');
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await api.patch(`/api/v1/funcionario/${params.id}/`, body);
    ...
    <SafeAreaView style={s.root} edges={['top']}>
      ...
      <View style={s.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={s.headerSub}>{nomeDisplay || 'Funcionário'}</Text>
        </View>
      </View>
        ...
        >
          ...
          >
            ...
            <View style={s.section}>
              ...
              <View style={s.card}>
                <Field      label="Função"          value={form.funcao}          onChange={set('funcao')}    placeholder="Ex: Recepcionista" error={fieldErr.funcao} />
