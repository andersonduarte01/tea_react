...
import {
  Bell, LogOut, Stethoscope, Briefcase, CalendarCheck,
  LockKeyhole, Menu, KeyRound, ArrowLeftRight, ChevronRight,
  AlertTriangle, User, Flag, HelpCircle,
...
function ClinicaLogo({ uri, imgStyle }: { uri: string | null; imgStyle?: object }) {
  ...
  if (dataUri) return <Image source={{ uri: dataUri }} style={[ic.clinicaFoto, imgStyle]} resizeMode="cover" />;
  return <LogoCircle />;
}
...
function ProfAvatar({ uri, nome }: { uri: string | null; nome: string }) {
  ...
  if (dataUri) return <Image source={{ uri: dataUri }} style={ic.profFoto} resizeMode="cover" />;
  return (
...
// ─── 1. HEADER ────────────────────────────────────────────────────────────────
function DashHeader({ name, clinicaNome, onMenuPress, subtitle }: {
  name:        string;
  clinicaNome: string;
  onMenuPress: () => void;
  subtitle?:   string;
...
function FloatingMetrics({ d }: { d: DashboardAdminData }) {
  const items = [
    { id: '1', icon: (c: string) => <Stethoscope  size={22} color={c} />, value: d.profissionais_total,    label: 'Profissionais' },
    { id: '2', icon: (c: string) => <Briefcase    size={22} color={c} />, value: d.funcionarios_presentes, label: 'Funcionários' },
    { id: '3', icon: (c: string) => <UsersIcon    size={22} color={c} />, value: d.pacientes_ativos,       label: 'Pacientes' },
    { id: '4', icon: (c: string) => <CalendarCheck size={22} color={c}/>, value: d.total_atendimentos ?? 0, label: 'Atendimentos' },
  ];
  return (
...
// ─── 3. CARD AZUL — resumo do dia ─────────────────────────────────────────────
function SummaryCard({ d, onAgendamentosPress, onFaltasPress }: {
  ...
  const funcText   = `${d.funcionarios_presentes} funcionário${d.funcionarios_presentes !== 1 ? 's' : ''}`;
      ...
      {temFaltas ? (
        <TouchableOpacity style={s.blueCardBottom} onPress={onFaltasPress} activeOpacity={0.75}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={s.blueCardPersonSub}>{funcText}</Text>
          </View>
          <View style={s.faltasBadgeRow}>
            <View style={[s.statusBadge, { backgroundColor: C.amber }]}>
              <Text style={s.statusText}>{d.faltas_hoje} falta{d.faltas_hoje > 1 ? 's' : ''} hoje</Text>
            </View>
...
function AlertsCard({ alertas }: { alertas: AlertaAPI[] }) {
...
// ─── CONTEÚDO PRINCIPAL ───────────────────────────────────────────────────────
function MainContent({ data, refreshing, onRefresh, onAgendamentosPress, onFaltasPress }: {
  ...
  return (
    ...
    >
      <FloatingMetrics d={data} />
      <SummaryCard d={data} onAgendamentosPress={onAgendamentosPress} onFaltasPress={onFaltasPress} />
      <AlertsCard alertas={data.alertas} />
...
export function FuncionarioDashboard() {
  ...
  const name        = user?.nome ?? 'Funcionário';
  ...
  const handleMeuPerfil = useCallback(() => {
    closeSidebar();
    if (user?.id) navigation.navigate('FuncionarioPerfil', { id: user.id });
  }, [closeSidebar, navigation, user?.id]);
  ...
  const handleTab = (key: TabKey) => {
    if (key === 'mais') { setMaisOpen(p => !p); return; }
    setActiveTab(key);
    setMaisOpen(false);
  };
  ...
  const renderContent = () => {
    ...
    return (
      <MainContent
        ...
        onRefresh={onRefresh}
        onAgendamentosPress={handleAgendamentos}
        onFaltasPress={handleFaltas}
      />
    );
  };
...
const s = StyleSheet.create({
  ...
  maisMenuIconWrap:{ width: 34, height: 34, borderRadius: 17, alignItems: 'center', justifyContent: 'center' },
  maisMenuLabel:   { fontSize: 15, fontWeight: '600' },
