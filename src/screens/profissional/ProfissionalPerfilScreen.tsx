...
type Route = RouteProp<AppStackParams, 'ProfissionalPerfil'>;
...
// ─── TELA PRINCIPAL ──────────────────────────────────────────────────────────
export function ProfissionalPerfilScreen() {
  const navigation = useNavigation<Nav>();
  const { params } = useRoute<Route>();
  const [prof,       setProf]       = useState<ProfissionalDetalhe | null>(null);
  ...
  const carregar = useCallback(async (isRefresh = false) => {
    ...
    setErro(null);
    try {
      const data = await api.get<ProfissionalDetalhe>(`/api/v1/profissional/${params.id}/`);
      setProf(data);
    } catch (err) {
      if (err instanceof SessionExpiredError) {
        setErro('Sessão expirada. Faça login novamente.');
      } else {
        setErro((err as any)?.message ?? 'Erro ao carregar profissional.');
      }
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  return (
    <SafeAreaView style={s.root} edges={['top']}>
      ...
      <View style={s.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={s.headerTitle}>Perfil do Profissional</Text>
          <Text style={s.headerSub}>Dados e informações</Text>
        </View>
        <TouchableOpacity
          style={s.editBtn}
          activeOpacity={0.7}
          onPress={() => navigation.navigate('ProfissionalEditar', { id: params.id })}
        >
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.7}
          onPress={() => navigation.navigate('ProfissionalNovaSenha', { id: params.id })}
        >
      ...
      {prof && !loading && (
        ...
        >
          {/* Hero — identidade do profissional */}
          <View style={s.heroCard}>
            <ProfFoto uri={prof.foto_url} nome={prof.nome} />
            <Text style={s.profNome}>{prof.nome}</Text>
            <View style={s.funcaoBadge}>
              <Briefcase size={13} color={C.primary} />
              <Text style={s.funcaoText}>{prof.funcao}</Text>
            </View>
            <View style={[s.statusBadge, prof.ativo ? s.statusAtivo : s.statusInativo]}>
              <Text style={[s.statusText, prof.ativo ? s.statusAtivoText : s.statusInativoText]}>
          ...
          <View style={s.section}>
            ...
            <View style={s.card}>
              <InfoRow icon={<User     size={18} color={C.primary} />} label="Nome completo"  value={prof.nome} />
              <View style={s.divider} />
              <InfoRow icon={<Briefcase size={18} color={C.primary} />} label="Função"        value={prof.funcao} />
              <View style={s.divider} />
              <InfoRow icon={<Mail     size={18} color={C.primary} />} label="E-mail"         value={prof.email} />
              <View style={s.divider} />
              <InfoRow icon={<Phone    size={18} color={C.primary} />} label="Telefone"       value={formatTelefone(prof.telefone)} />
              <View style={s.divider} />
              <InfoRow icon={<CreditCard size={18} color={C.primary} />} label="CPF"          value={formatCpf(prof.cpf)} />
              <View style={s.divider} />
              <InfoRow icon={<Calendar size={18} color={C.primary} />} label="Data de nascimento" value={formatDate(prof.data_nascimento)} />
              <View style={s.divider} />
              <InfoRow icon={<ShieldCheck size={18} color={C.primary} />} label="Status"      value={prof.ativo ? 'Ativo' : 'Inativo'} />
            </View>
          </View>
            ...
            <View style={s.section}>
              ...
              <View style={s.card}>
                <InfoRow
                  ...
                  label={`${prof.endereco.cidade}/${prof.endereco.estado}`}
                  value={`${prof.endereco.logradouro}, ${prof.endereco.numero}${prof.endereco.complemento ? ` — ${prof.endereco.complemento}` : ''}\n${prof.endereco.bairro} · ${prof.endereco.cidade}/${prof.endereco.estado}\nCEP ${prof.endereco.cep}`}
                />
              </View>
            </View>
          )}
