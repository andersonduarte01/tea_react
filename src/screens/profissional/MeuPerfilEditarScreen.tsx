...
// ─── Screen principal ─────────────────────────────────────────────────────────
export function MeuPerfilEditarScreen() {
  ...
  const [form, setForm] = useState<MeuForm>({
    telefone: '', data_nascimento: '',
    logradouro: '', numero: '', complemento: '',
    bairro: '', cidade: '', estado: '', cep: '',
  });

  const set = (key: keyof MeuForm) => (v: string) =>
    setForm(f => ({ ...f, [key]: v }));
  ...
  const carregar = useCallback(async () => {
    ...
    setErro(null);
    try {
      const data = await api.get<any>('/api/v1/profissional/perfil/');
      setNomeDisplay(data.nome ?? '');
      setServerPhotoUri(data.foto_url ?? data.foto ?? null);
      setForm({
  ...
  const uploadFoto = async (uri: string) => {
    ...
    try {
      ...
      await api.patchMultipart('/api/v1/profissional/perfil/', formData);
      setLocalPhotoUri(uri);
      Alert.alert('Foto atualizada', 'Sua foto foi atualizada com sucesso.');
    } catch (err) {
      if (err instanceof SessionExpiredError) {
        setErro('Sessão expirada. Faça login novamente.');
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await api.patch('/api/v1/profissional/perfil/', body);
      Alert.alert('Sucesso', 'Perfil atualizado com sucesso.', [
        { text: 'OK', onPress: () => navigation.navigate('MeuPerfilProfissional') },
      ]);
    } catch (err: any) {
      if (err instanceof SessionExpiredError) {
        setErro('Sessão expirada. Faça login novamente.');
  ...
  return (
    <SafeAreaView style={s.root} edges={['top']}>
      ...
      <View style={s.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={s.headerTitle}>Editar Meu Perfil</Text>
            ...
            <View style={s.section}>
              ...
              <View style={s.card}>
                <Field
                  ...
                  error={fieldErr.telefone}
                />
                <DateField
                  label="Data de nascimento"
                  value={form.data_nascimento}
                  onChange={set('data_nascimento')}
                  error={fieldErr.data_nascimento}
                />
              </View>
            </View>
