...
export interface UseAgendamentosTodosReturn extends State {
  refresh:  () => Promise<void>;
  loadMore: () => Promise<void>;
}

export function useAgendamentosHojeTodos(): UseAgendamentosTodosReturn {
  ...
  const load = useCallback(async (pageNum: number, reset: boolean) => {
    ...
    try {
      const data = await getAgendamentosHojeTodos(pageNum);
      setState(prev => ({
